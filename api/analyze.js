...
${carfaxText.substring(0, 14000)}`;
  ...
  try {
    ...
    const mechDamageVariants = [
      ...
      'Se reporta daño mecánico en la unidad, que podría involucrar el motor, la transmisión u otros componentes. Sugerimos tomarlo en cuenta al evaluar el vehículo.'
    ];
    ...
    const excelentePool = (damageList.length > 0 || hasMechDamage) ? excelenteVariantsConDanos : excelenteVariantsSinDanos;
...
` : ''}
Reglas:
${esBillOfSale ? `- CASO ESPECIAL (Bill of Sale): NO menciones el título, NO uses la palabra "Bill of Sale" y NO expliques ningún trámite. La explicación del Bill of Sale se agrega automáticamente por separado. Escribe SOLO los daños${miles ? ' y las millas' : ''} en una frase natural. Si no hay daños${miles ? '' : ' ni millas'} que mencionar, responde con una cadena vacía.` : `- Empieza indicando el título sin afirmarlo con certeza absoluta, atribuyéndolo a la subasta, usando la frase "Certificado de Título ${titleType}" (NO solo "título ${titleType}"). VARÍA la forma de decirlo cada vez, usa diferentes opciones como: "La subasta indica Certificado de Título ${titleType}", "El lote figura con Certificado de Título ${titleType}", "${auction} reporta Certificado de Título ${titleType}", "El vehículo aparece listado con Certificado de Título ${titleType}", "Registrado en la subasta como Certificado de Título ${titleType}". NUNCA uses siempre la misma frase, NUNCA digas "El título de ${titleType}".
- IMPORTANTE sobre redundancia: nombra la fuente UNA SOLA VEZ en toda la oración. Si ya dijiste "${auction || 'la subasta'}" o "Copart"/"IAAI"/"Manheim" al inicio, NO vuelvas a decir "la subasta" al explicar el significado — usa un conector directo como "lo que refiere que..." o "lo que quiere decir que...". Solo di "según la subasta" o "según ${auction}" si es la PRIMERA vez que la mencionas en la oración.
...
- PROHIBIDO escribir "este título". Es la fórmula que más se repite y suena robótica. Para retomar el título antes del verbo, VARÍA en cada generación entre estas opciones: "dicho título", "esta clasificación", "dicha clasificación", "esta condición", "dicha categoría", o simplemente NO retomarlo y encadenar directo ("lo que refiere que..."). Elige una distinta cada vez.
...
- Devuelve SOLO el texto mejorado, sin comillas ni preámbulo.` }],
    ...
    if (titleType && !esBillOfSale && !soloFechaFuturo) {
      ...
      if (!mencionaTitulo) {
        const fuente = auction || 'la subasta';
        const tituloFallbackVariants = [
          `La subasta indica Certificado de Título ${titleType}, lo que refiere que ${titleExplain[titleType] || ''}.`,
          `El lote figura con Certificado de Título ${titleType}, lo que de acuerdo a ${fuente}, esta clasificación refiere que ${titleExplain[titleType] || ''}.`,
          `${fuente} reporta Certificado de Título ${titleType}, lo que refiere que ${titleExplain[titleType] || ''}.`
        ];
